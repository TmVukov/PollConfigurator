...
import { PostResponse, PollRequest, PollResponse, Question } from './poll.model';
...
export class PollService {
  ...
  // SAKRITI URL!!!!
  private url = 'https://habit-reminder-653cd.firebaseio.com';
  ...
  saveData(payload: PollRequest): void {
    this.http.post<PostResponse>(`${this.url}/question.json`, payload).subscribe(data => console.log('post', data));
  }
  ...
  deleteData(id: string): Observable<PollResponse[]>  {
    ...
    // return this.http.delete<PollResponse[]>(`${this.url}/question/${id}/questions/${i}.json`);
  }
